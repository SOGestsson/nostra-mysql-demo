#!/usr/bin/env python3
"""Add vendor_info.is_manual column so manual vendors survive data sync."""
...
def migrate_vendor_manual_schema(conn) -> None:
    with conn.cursor() as cursor:
        cursor.execute("SHOW TABLES LIKE 'vendor_info'")
        if not cursor.fetchone():
            print("  skip: vendor_info table missing")
            return
        cursor.execute("SHOW COLUMNS FROM vendor_info LIKE 'is_manual'")
        if not cursor.fetchone():
            cursor.execute(
...
def main() -> int:
    ...
    try:
        ...
        for logical_name, physical in targets:
            ...
            try:
                migrate_vendor_manual_schema(data_conn)
